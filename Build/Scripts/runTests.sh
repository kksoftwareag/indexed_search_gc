...
read -r -d '' HELP <<EOF
indexed_search_gc test runner. Execute unit, functional and other test suites in
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/indexed_search_gc/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="Web/typo3conf/ext/indexed_search_gc/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/indexed_search_gc/Tests/Unit"
            ;;
    esac
fi
